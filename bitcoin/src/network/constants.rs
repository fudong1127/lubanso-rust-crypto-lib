...
use lubanso_internals::{debug_from_display, write_err};
#[cfg(feature = "serde")]
...
impl fmt::Display for Magic {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        lubanso_internals::fmt_hex_exact!(f, 4, &self.0, lubanso_internals::hex::Case::Lower)?;
        Ok(())
    }
}
...
impl fmt::UpperHex for Magic {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        lubanso_internals::fmt_hex_exact!(f, 4, &self.0, lubanso_internals::hex::Case::Upper)?;
        Ok(())
    }
}
