...
//!
//! ```rust
//! # #[cfg(all(feature = "rand-std", feature = "lubanso-hashes-std"))] {
...
//!
//! ```rust
//! # #[cfg(all(feature = "global-context", feature = "lubanso-hashes-std", feature = "rand-std"))] {
...
//! ```
//!
//! The above code requires `rust-secp256k1` to be compiled with the `rand-std` and `lubanso-hashes-std`
...
//! // See the above example for how to use this library together with `lubanso-hashes-std`.
...
//! * `lubanso-hashes` - use the `lubanso-hashes` library.
//! * `lubanso-hashes-std` - use the `lubanso-hashes` library with its `std` feature enabled (implies `lubanso-hashes`).
...
#[cfg_attr(docsrs, doc(cfg(feature = "lubanso-hashes")))]
pub use lubanso_hashes as hashes;
...
pub use rand;
pub use lubanso_secp256k1_sys as ffi;
#[cfg(feature = "serde")]
...
impl Message {
    ...
    ///
    /// Requires the feature `lubanso-hashes` to be enabled.
    ///
    /// # Examples
    ///
    /// ```
